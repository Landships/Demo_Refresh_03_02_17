...
public class Cannon_Base_CSEditor : Editor {
	...
	void Create () {
		...
		int Child_Count = 0 ;
		if ( Parent_Transform.Find ( "Cannon" ) ) {
			Temp_Object = Parent_Transform.Find ( "Cannon" ).gameObject ;
		...
		GameObject Part_Object = new GameObject ( "Cannon" ) ;
		...
		Temp_Damage_Control_CS.Type = 3 ;  // 3 = Cannon in "Damage_Control"
